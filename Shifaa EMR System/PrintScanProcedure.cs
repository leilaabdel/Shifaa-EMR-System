...
namespace Shifaa_EMR_System
{
    ...
    public partial class PrintScanProcedure : Form
    {
        ...
        public PrintScanProcedure(int patientID, string providerID)
        {
            InitializeComponent();
        ...
        private void SetScans(RichTextBoxEx report, DateTime date)
        {




            ISingleResult<getScheduledProcedureResult> result = doAction.getScheduledProcedure(thisPatientID, date);
            foreach (getScheduledProcedureResult r in result)
            {
                report.SelectionAlignment = HorizontalAlignment.Left;
                Console.WriteLine("0");
                report.SelectionBullet = true;
                report.SelectionIndent = 60;
                Console.WriteLine(r.ScanName);
                report.SelectionFont = new Font("Bahnschrift Light", 14);
                report.SelectedText = r.ScanName + "\n";
